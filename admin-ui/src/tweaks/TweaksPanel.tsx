...
export function TweakRadio({ label, value, options, onChange }: { label: string; value: string; onChange: (v: string) => void; options: { value: string; label: string }[] }) {
  const trackRef = useRef<HTMLDivElement>(null);
  const [dragging, setDragging] = useState(false);
  const valueRef = useRef(value);
  valueRef.current = value;

  const labelLen = (o: { value: string; label: string }) => String(o.label).length;
  ...
  if (!fitsAsSegments) {
    const resolve = (s: string) => {
      const m = options.find((o) => o.value === s);
      return m === undefined ? s : m.value;
    };
    return (
      <TweakRow label={label}>
        <select className="twk-field" value={value} onChange={(e) => onChange(resolve(e.target.value))}>
          {options.map((o) => (
...
export function TweakColor({ label, value, options, onChange }: { label: string; value: string | string[]; onChange: (v: string | string[]) => void; options: (string | string[])[] }) {
  ...
  const key = (o: string | string[] | undefined) => String(JSON.stringify(o)).toLowerCase();
  const cur = key(value);
  return (
    <TweakRow label={label}>
