 ...
 *
 * - ASCII 영숫자만 먼저 통과 → 유니코드 case-fold 가 알파벳 문자를 합성하는 것을 차단
...
export function formatRecoveryCode(input: string): string {
  // ASCII 영숫자만 먼저 통과 → 유니코드 case-fold(예: 'ß'.toUpperCase() === 'SS')가
  // 알파벳 문자를 합성하는 것을 차단한다.
  const ascii = input.replace(/[^A-Za-z0-9]/g, '');
