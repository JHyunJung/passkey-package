...
/**
 * Reads + writes the {@code <schema>.mds_sync_history} append-only log
 * (V34 migration). Uses {@link JdbcTemplate} with an explicit schema prefix
 * injected from {@link DbSchemaProperties} — matching the existing pattern in
 * {@link MdsBlobStore} and {@link MdsAdminController}.
 *
 * <p>Note: the Hibernate {@code {h-schema}} placeholder is <b>not</b> substituted
 * in raw JdbcTemplate SQL (only inside Hibernate-managed native queries), so the
 * schema comes from configuration, same as the rest of this package.
 */
@Service
public class MdsHistoryService {
     ...
     * recent()/append() 와 동일하게 주입된 스키마 prefix 명시(JdbcTemplate raw SQL).
     *
    ...
    public int purgeStartedBefore(OffsetDateTime cutoff, int batchSize) {
        ...
        return jdbc.update(
                "DELETE FROM " + schema + ".mds_sync_history WHERE started_at < ? AND ROWNUM <= ?",
    ...
    public int successRate30dCountOk() {
        ...
        Integer n = jdbc.queryForObject(
                "SELECT COUNT(*) FROM " + schema + ".mds_sync_history WHERE status='SYNCED' AND started_at >= ?",
                Integer.class,
    ...
    public int successRate30dCountTotal() {
        Instant since = Instant.now().minus(Duration.ofDays(30));
        Integer n = jdbc.queryForObject(
                "SELECT COUNT(*) FROM " + schema + ".mds_sync_history WHERE started_at >= ?",
                Integer.class,
                java.sql.Timestamp.from(since));
        return n == null ? 0 : n;
    }
