...
앱/웹 클라이언트 → **RP 서버** API 한눈 요약. 흐름·구현 가이드는 [rp-client-api.md](./rp-client-api.md) 참조.

- **Base URL**: `https://dev-passkey.crosscert.com` (로컬 단독 개발 시 `http://localhost:8090`)
...
| | `excludeCredentials` | array | `[{ type, id(B64URL), transports(string[]) }]` (없으면 생략) |
...
| | `transports` | string[] | `cred.response.getTransports()` 결과 배열 그대로 (없으면 `[]`) |
...
| | `timeout` | number | ms |
| | `rpId` | string | |
| | `allowCredentials` | array | `[{ type, id(B64URL), transports(string[]) }]` |
...
- `transports`는 등록 시 `cred.response.getTransports?.() ?? []` 결과를 배열 그대로 전송 (인증 단계엔 보내지 않음).
