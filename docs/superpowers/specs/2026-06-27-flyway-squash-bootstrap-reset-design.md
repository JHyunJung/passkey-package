...
3. 정리된 부트스트랩 스크립트 — VPD 죽은 주석 제거 + `XEPDB1` → `${ORA_SERVICE}` 매개변수화.
4. 검증 통과 — 실DB A-vs-B 스키마 diff = 0 + 기존 IT 그린.

> **참고**: R__ 시드 파일(`R__seed_operators`, `R__seed_dev_tenant` 등)은 **변경하지 않는다**. 운영자 계정 시드는 이미 versioned 마이그레이션 밖에 있어 squash 영향을 받지 않는다 (§4.② 참조).
...
### ② 운영자 계정 시드 → 이미 R__ repeatable 로 일원화됨 (조치 불필요)
계획 착수 시 코드 재확인 결과, 운영자 계정 시드는 **이미 versioned 마이그레이션 밖(R__)에 있다.** squash 가 versioned(V1~V52)만 건드리므로 시드는 그대로 보존된다.
- `V11__seed_admin_user.sql` 은 **이미 no-op**(`SELECT 1 FROM dual`) — alice 시드는 진작 `R__seed_operators.sql`(seed-common)로 이전됨.
- **bob(RP_ADMIN)** 은 V29 가 아니라 `db/dev/R__seed_dev_tenant.sql:145`(dev 프로필) + `db/testfix/V9001`(테스트)에 있다. RP_ADMIN 은 V23 CHECK 상 `tenant_id NOT NULL` 이라 dev 테넌트와 함께 시드되어야 하기 때문. (alice = `R__seed_operators`, bob = dev 시드 — 의도된 분리)
- **결정**: V1 baseline 에는 운영자 계정 INSERT 를 넣지 않는다(현 상태 유지). R__ 시드 파일은 손대지 않는다. prod 는 seed 미포함 그대로.
- 따라서 **§2 산출물 3(R__seed_operators 갱신)은 불필요** — 시드 이전 작업 없음.
...
| testfix V9000/V9001 충돌 | db/testfix 격리, V1 뒤 적용 — 무영향(확인됨) |
