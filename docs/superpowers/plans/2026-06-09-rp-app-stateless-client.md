...
## Task 8: P0-4 결정적 등록 confirm + P2 (relay 키 가드, 문서 envelope) — 최종 리뷰 반영

**배경:** 최종 통합 codex 리뷰가 P1 1건 + P2 2건을 냈다(spec §5 갱신 반영).
- **P0-4**: register/finish 가 relay 의 userHandle 을 `confirmRegistration` 에 넘기는데, 그 handle 이 메모리 pending 에 없으면(`computeIfPresent`) **조용히 no-op** → passkey-app 은 credential 생성, rp-app 은 "성공" 반환, 로컬 매핑 누락 → 이후 로그인 unknown-sub. rp-app 재시작·다중 인스턴스에서 발생. 레퍼런스 RP 가 가르치면 안 될 패턴.
- **P2-a**: `RelayProperties` 데모 기본 HMAC 키 폴백 — 운영이 데모 키로 조용히 뜰 위험. P0-3/P0-4 위조불가가 이 키에 의존.
- **P2-b**: `docs/external-client-integration.md` 가 응답을 raw shape 로 표기하나 실제는 `ApiResponse` envelope(`data.*`). 또 "regRelayToken 파싱 불가"는 부정확(디코드는 됨, HMAC 키 없이 수정만 불가).

**Files:**
- Modify: `rp-app/.../web/relay/RegRelayCodec.java` (payload 에 username/displayName 추가)
- Modify: `rp-app/.../user/InMemoryUserStore.java` (결정적 confirm 메서드)
- Modify: `rp-app/.../web/WebAuthnController.java` (relay encode/decode + 결정적 confirm 호출)
- Modify: `rp-app/.../config/RelayProperties.java` 또는 신규 startup 가드 (P2-a)
- Modify: `rp-app/.../RpAppApplication.java` 또는 설정 (non-dev 프로필 데모 키 차단)
- Modify: `docs/external-client-integration.md` (P2-b)
- Modify: `rp-app/src/test/.../{WebAuthnControllerTest,RegRelayCodecTest}.java` (새 payload/결정적 confirm 검증)

- [ ] **Step 1: RegRelayCodec payload 확장 (username/displayName)**

`RegRelayCodec` 의 `RegRelay` record 와 `ObjectNodePayload` 에 username/displayName 추가:
```java
    /** 복원된 relay payload. */
    public record RegRelay(String registrationToken, String userHandle, String username, String displayName) {}
```
`encode` 시그니처를 `encode(String registrationToken, String userHandle, String username, String displayName)` 로, payload record 를 `record ObjectNodePayload(String rt, String uh, String un, String dn, long exp) {}` 로. encode/decode 본문을 4필드로 갱신(decode 가 RegRelay 4-arg 반환). 나머지(HMAC, 상수시간 비교, 만료)는 그대로.

- [ ] **Step 2: InMemoryUserStore 결정적 confirm 메서드 추가**

기존 `confirmRegistration(userHandle, credentialId)` 옆에 결정적 버전 추가(있으면 갱신, 없으면 relay 정보로 생성):
```java
    /**
...
```
기존 3-arg `confirmRegistration(userHandle, credentialId)` 은 호출처가 없어지면 제거(grep 확인 후). (테스트가 쓰면 함께 정리.)

- [ ] **Step 3: WebAuthnController register/begin·finish 갱신**

begin: `relay.encode(sdkResp.registrationToken(), userHandle, req.username(), req.displayName())`.
finish: relay decode 후 `users.confirmRegistration(r.userHandle(), r.username(), r.displayName(), fin.credentialId())`.

- [ ] **Step 4: relay 데모 키 startup 가드 (P2-a)**

`RelayProperties` 가 데모 기본 키로 폴백할 때, 활성 프로필이 local/dev 가 아니면 부팅 실패(또는 강한 경고). 구현: `RelayProperties` 에 `@Value("${spring.profiles.active:}")` 주입은 record 라 어색하므로, 별도 `@Component` `RelayKeyGuard` (ApplicationRunner 또는 @PostConstruct) 에서 `Environment.getActiveProfiles()` 와 secret 데모 기본값 일치를 검사해 non-dev/local 이면 `IllegalStateException`. 데모 기본 키 상수는 `RelayProperties` 와 공유.
```java
package com.crosscert.passkey.rpapp.config;
...
public class RelayKeyGuard {
    ...
    public RelayKeyGuard(RelayProperties props, Environment env) { this.props = props; this.env = env; }
...
```
`RelayProperties` 의 데모 기본값을 `RelayKeyGuard.DEMO_SECRET` 와 동일 문자열로 맞춘다(상수 공유 위해 RelayProperties 가 `RelayKeyGuard.DEMO_SECRET` 참조하거나 그 반대 — 순환 피해 한쪽에 상수 두고 다른 쪽이 참조).

- [ ] **Step 5: 문서 P2-b 수정**

`docs/external-client-integration.md`:
- 응답 표기를 envelope 로: "응답 `ApiResponse` 형식 — 실제 값은 `data` 아래. 예: `data.publicKeyCredentialCreationOptions`, `data.regRelayToken`, `data.authenticationToken`, `data.authenticated`."
- regRelayToken 설명 정정: "불투명 토큰이지만 base64url payload 라 **디코드(열람)는 가능**하다. HMAC 키가 없으면 **수정(위조)이 불가**할 뿐이다. 민감정보를 담지 않으며(등록 토큰·userHandle·username·displayName), 무결성만 서명으로 보장한다."

- [ ] **Step 6: 테스트 갱신**

- `RegRelayCodecTest`: encode/decode 를 4필드로, round-trip 이 username/displayName 도 복원하는지. 서명변조 시 username 도 보호되는지(기존 payload-tamper 테스트 유지).
- `WebAuthnControllerTest`: register/finish 가 relay 의 username/displayName 으로 `confirmRegistration(uh, un, dn, credId)` 호출하는지 verify. **결정적 생성 회귀 가드**: store 에 pending 이 없는 상태(빈 store)에서 finish 해도 user 가 생성·확정되는지(InMemoryUserStore 단위 테스트로: `confirmRegistration("h","u","d","cred")` 후 `findByUserHandle("h")` 가 credentialId 채워진 user 반환).
- `RelayKeyGuard` 테스트(선택): non-dev 프로필 + 데모 키 → IllegalStateException, dev 프로필 → 통과.

- [ ] **Step 7: 빌드 + 테스트**

Run: `./gradlew :rp-app:compileJava :rp-app:test -q`
Expected: BUILD SUCCESSFUL, 모든 테스트 PASS.

- [ ] **Step 8: 커밋**

```bash
git add rp-app/ docs/external-client-integration.md
git commit -m "fix(rp-app): 결정적 등록 confirm(P0-4)·relay 키 가드(P2)·문서 envelope 정정 (최종 리뷰)"
```
