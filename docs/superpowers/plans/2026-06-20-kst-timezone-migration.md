...
> **Task 5 에서 넘어온 필수 수정 (런타임 ClassCastException — 반드시 고칠 것):** `admin-app/.../tenant/TenantAdminService.java` 의 `toView` 가 `AuditLog.getCreatedAt()`(이제 `OffsetDateTime`)에서 `lastEventAt` 을 끌어오는데 `Instant lastEventAt` 으로 받고, `toInstantMap`(약 line 92-98)이 `findLatestCreatedAtGroupedByTenantId()` 의 `Object[]` 결과를 `(Instant)` 로 캐스트한다 — Hibernate 가 이제 `OffsetDateTime` 을 반환하므로 **런타임 ClassCastException**. `TenantAdminDto.TenantView` 의 `lastEventAt` 포함 경로를 `OffsetDateTime` 으로 바꾸거나 값에 `.toInstant()` 적용. 이건 audit 슬라이스에서 발원해 tenant 패키지 파일에 착지한 ripple 이라 Task 7(tenant)에서 처리. 컴파일 에러(line 117/118)로도 드러나므로 Task 7 이 tenant 를 손대면 자연히 마주친다 — 단 `(Instant)` 캐스트는 컴파일은 통과하고 런타임에만 터지므로 명시적으로 잡을 것.
