...
| ID | 항목 | 비고 |
|---|---|---|
| P1-1 | per-tenant WebAuthn 설정값 ceremony 미반영 (timeout/UV/attestation conveyance) | 컬럼 존재, start 단계 하드코딩 |
...
| P1-5 | API key rotation & scope 강제 | revoke만 가능, scope 저장만 |
| P1-6 | Admin password reset / account lockout | invitation flow만 존재 |
...
### 2.1 Admin MFA recovery-code flow (P0-5 일부)
- **현황**: V36 마이그레이션이 `ADMIN_USER_RECOVERY_CODE` 테이블을 생성했으나 **엔티티·repository·발급/소비 로직 없음** (schema-only, 미사용 ship).
- **영향**: authenticator 분실 시 운영자가 스스로 복구 불가 → 현재는 admin이 DB에서 `mfa_enabled='N'` 직접 변경 필요.
- **권고**: recovery code 발급(enroll 시 N개 생성, sha-256 hash만 저장) + verify 대체 소비 흐름. 테이블은 이미 준비됨.

### 2.2 `mfa_secret` 평문 저장 (P0-5)
- **현황**: TOTP secret이 `admin_user.mfa_secret`에 평문 Base32로 저장 (V32 스키마 그대로).
- **영향**: DB 유출 시 TOTP secret 노출 → MFA 우회 가능.
- **권고**: 프로젝트의 `KeyEnvelope`(AES-256-GCM, signing key에 이미 사용)로 at-rest 암호화. **단, sealed+base64는 ~60~80자라 `VARCHAR2(64)` 초과 → 컬럼 확장 마이그레이션 필요.** "SaaS launch hardening" 배너 하의 at-rest 후속으로 적합.
