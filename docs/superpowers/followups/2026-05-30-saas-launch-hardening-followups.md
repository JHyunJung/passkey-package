...
| ~~P1-2~~ | ~~Metrics 파이프라인 (Prometheus/Micrometer)~~ | ✅ **해결 (2026-05-31, 그룹 C)** — micrometer-registry-prometheus + `/actuator/prometheus` + 도메인 counter(ceremony/apikey/mds), cardinality 규율 |
| ~~P1-3~~ | ~~Alerting hook (email/Slack/PagerDuty)~~ | ✅ **해결 (2026-05-31, 그룹 C)** — ApplicationEventPublisher → @Async AlertDispatcher → AlertChannel(Log 기본+Mail 설정형), 5개 보안 이벤트 발행 |
| ~~P1-4~~ | ~~Audit / 데이터 retention 정책~~ | ✅ **해결 (2026-05-31, 그룹 D)** — RetentionPurgeJob(@Scheduled+lease), 5개 무한증가 테이블 배치 purge(토큰3+히스토리2), 활성 데이터 보존. audit_log는 hash-chain forensic 제외 |
...
| ~~P1-7~~ | ~~Health check 보강 (DB/Redis)~~ | ✅ **해결 (2026-05-31, 그룹 C)** — health probes(readiness/liveness) 분리 + readiness 그룹에 db·redis 포함(설정) |
...
>
> **그룹 C 완료 (2026-05-31)**: P1-2(Prometheus metrics) + P1-3(event-based alerting) + P1-7(health probes)를 단일 worktree로 마감(신규 마이그레이션 없음). spec: `2026-05-31-observability-group-c-design.md`, followups: `2026-05-31-observability-group-c-followups.md`.
>
> **그룹 D 완료 (2026-05-31)**: P1-4(데이터 retention/purge)를 단일 worktree로 마감(신규 마이그레이션 없음). RetentionPurgeJob — 5개 테이블 배치 purge. spec: `2026-05-31-data-retention-group-d-design.md`, followups: `2026-05-31-data-retention-group-d-followups.md`.
>
> **✅ P1 7건 전부 완료 (P1-1~7)**. 남은 SaaS readiness: P2 8건(spec §5), 별개(RP_ADMIN role 게이팅, admin-ui 후속).
