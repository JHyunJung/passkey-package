 ...
 * 요구하는 {@code UPDATED_AT} 컬럼이 없다. 따라서 SchedulerLease 와 동일하게
 * Oracle DEFAULT 에 의존하는 독립 매핑을 사용한다.
 *
 * <p>id/created_at 은 DB DEFAULT(SYS_GUID()/SYSTIMESTAMP) 가 채우므로
 * insert 문에서 제외하고({@code insertable = false}), insert 후 즉시 되읽어온다
 * ({@code @Generated(INSERT)}).
 */
@Entity
