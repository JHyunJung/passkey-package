...
--
--    [2단계] APP_OWNER 로 실행 — 테이블/인덱스/뷰/패키지/객체권한 생성
--      sqlplus APP_OWNER/<pw>@<host>:1521/<service> @full-schema.sql
--
...
--      scripts/full-schema-part2-schema.sql 을 APP_OWNER 로 실행할 것.
--
-- ############################################################################
-- ############################################################################
...
-- ============================================================
--   APP_OWNER        : 스키마 소유자. DDL 권한 보유. Flyway 마이그레이션 전용.
--   APP_RUNTIME_USER : passkey-app 런타임. 테이블별 GRANT 만. DDL 불가.
--   APP_ADMIN_USER   : admin-app 런타임. 관리 테이블 추가 권한. DDL 불가.
--
-- 앱이 접속하는 두 유저에서 DDL 을 제거해, SQL 인젝션이나 버그로 DROP TABLE
-- 이 실행되더라도 권한 부족으로 실패하게 만든다. 스키마 변경은 배포 시점에
-- APP_OWNER 자격증명으로만 가능하다.
--
...
-- APP_RUNTIME: 런타임 세션(passkey-app, admin-app 일반 트랜잭션)
BEGIN
...
-- APP_ADMIN: admin-app 런타임 + 스케줄러. 크로스테넌트 조회는 앱 레이어에서
...
BEGIN
  EXECUTE IMMEDIATE 'CREATE USER APP_RUNTIME_USER IDENTIFIED BY "&app_runtime_pw"';
EXCEPTION WHEN OTHERS THEN
...
BEGIN
  EXECUTE IMMEDIATE 'CREATE USER APP_ADMIN_USER IDENTIFIED BY "&app_admin_pw"';
EXCEPTION WHEN OTHERS THEN
  IF SQLCODE = -1920 THEN NULL;
  ELSE RAISE;
  END IF;
...
PROMPT  이제 APP_OWNER 로 접속해 PART 2 를 실행하라.
PROMPT ============================================================
PROMPT
